...
- For cursor-paginated endpoints (`/users`, `/applications`, `/access_states`,
  ...
  across the run. A null or absent cursor proves exhaustion only
  when a valid, stable, nonnegative integer `meta.total_count` equals the
  accumulated count. Require `page` to advance by one, `page_size` to be 1
  through 100, stable nonnegative `total_pages`, page data no longer than
  `page_size`, and arithmetic consistency among page, page size, total pages,
  total count, and accumulated count. A zero total may use zero or one total
  page. Stop if a total is missing, malformed, changes, or
  is exceeded, a cursor or record ID repeats, a page fails, or no field proves exhaustion.
